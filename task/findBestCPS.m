...
CPS_all = {};
for q = 1:100000
    CPS{1}.probe = randperm(n_electrode, 1);
    CPS{1}.first = randperm(n_electrode, 1);
    CPS{1}.second = randperm(n_electrode, 1);
    CPS{1}.interval_1 = min_isi+(max_isi-min_isi)*rand();
    CPS{1}.interval_2 = min_isi+(max_isi-min_isi)*rand(); 
    trace_pre = struct();
    
    for run = 1:5
%         title(sprintf('run: %d/5, CPS: %d', run, q));
...
%         dot_animat.MarkerFaceColor = color_animat(1,:);
...
%         drawnow;
        ...
        while dist2origin<=r_outer
            ...
            [stim_CPS, time_probe, duration] = getCPS(min_isi, max_isi, CPS, 1, stimulator, dt, stimulus_amplitude, stimulus_duration);
            ...
            disp(sum(firing_rate==0))
            CA = 2*getCA(firing_rate, col_electrode, row_electrode);
...
%                 'Marker', 'o', 'MarkerFaceColor', color_animat(1,:), 'MarkerEdgeColor', 'none', 'MarkerSize', 2);
% 
%             uistack(dot_animat, 'top')
% 
%             drawnow;
            trace_pre(run).animat = trace_animat_pre;
            trace_pre(run).CA = trace_CA_pre;            
        end

    end
    CPS{1}.trace = trace_pre;
    CPS_all{length(CPS_all)+1} = CPS{1};
    CPS_all
    save('CPS_all','CPS_all')
end
clc;
